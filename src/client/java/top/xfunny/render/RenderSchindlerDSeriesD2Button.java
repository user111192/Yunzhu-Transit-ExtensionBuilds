...
public class RenderSchindlerDSeriesD2Button  extends BlockEntityRenderer<SchindlerDSeriesD2Button.BlockEntity> implements DirectionHelper, IGui, IBlock {
	...
	public void render(SchindlerDSeriesD2Button.BlockEntity blockEntity, float tickDelta, GraphicsHolder graphicsHolder1, int light, int overlay){
		...
		blockEntity.forEachTrackPosition(trackPosition -> {
			...
			SchindlerDSeriesD2Button.hasButtonsClient(trackPosition, buttonStates, (floorIndex, lift) -> {
			...
			MainRenderer.scheduleRender(
					...
					(graphicsHolder, offset) -> {
						...
						IDrawing.drawTexture(
								...
								1,
								facing,
								buttonStates[2] ? PRESSED_COLOR : lookingAtBottomHalf ? HOVER_COLOR : 0x00B8FD95,
								light
						);
						// 弹出当前图形状态
						graphicsHolder.pop();
			...
			MainRenderer.scheduleRender(
					...
					(graphicsHolder, offset) -> {
						...
						IDrawing.drawTexture(
								...
								0,
								facing,
								buttonStates[3] ? PRESSED_COLOR : lookingAtTopHalf ? HOVER_COLOR : 0x00B8FD95,
								light
						);
						// 弹出当前图形状态
						graphicsHolder.pop();
