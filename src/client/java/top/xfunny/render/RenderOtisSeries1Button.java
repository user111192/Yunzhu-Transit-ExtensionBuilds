	...
	public void render(OtisSeries1Button.BlockEntity blockEntity, float tickDelta, GraphicsHolder graphicsHolder1, int light, int overlay){
		...
		blockEntity.forEachTrackPosition(trackPosition -> {
			...
			if (world.getBlockState(trackPosition).getBlock().data instanceof BlockLiftTrackFloor) {
				...
				top.xfunny.render.RenderLiftObjectLink.RenderLiftObjectLink(
		...
		if (buttonDescriptor.hasDownButton()) {
			...
			MainRenderer.scheduleRender(
					BUTTON_TEXTURE,
					false,
					buttonStates[0] || lookingAtBottomHalf ? QueuedRenderLayer.LIGHT_TRANSLUCENT : QueuedRenderLayer.INTERIOR,
					(graphicsHolder, offset) -> {
						...
						IDrawing.drawTexture(
								graphicsHolder,
								-1.25F / 16,
								(buttonDescriptor.hasUpButton() ? 2.9F : 3.5F) / 16,
								1F / 16,
								1F / 16,
								0,
								0,
								...
								1,
								facing,
								buttonStates[0] ? PRESSED_COLOR : lookingAtBottomHalf ? HOVER_COLOR : 0xFFFFFFFF,
								light
						);
						// 弹出当前图形状态
						graphicsHolder.pop();
			...
			MainRenderer.scheduleRender(
					...
					(graphicsHolder, offset) -> {
						...
						IDrawing.drawTexture(
								graphicsHolder,
								-1.25F / 16,
								(buttonDescriptor.hasDownButton() ? 4.1F : 3.5F) / 16,
								1F / 16,
								1F / 16,
								0,
								1,
								...
								0,
								facing,
								buttonStates[1] ? PRESSED_COLOR : lookingAtTopHalf ? HOVER_COLOR : 0xFFFFFFFF,
								light
						);
