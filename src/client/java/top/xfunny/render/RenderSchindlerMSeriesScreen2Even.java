...
public class RenderSchindlerMSeriesScreen2Even extends BlockEntityRenderer<SchindlerMSeriesScreen2Even.BlockEntity> implements DirectionHelper, IGui, IBlock {
	...
	public RenderSchindlerMSeriesScreen2Even(Argument dispatcher, Boolean isEven) {
		super(dispatcher);
		this.isEven = isEven;
	}
	@Override
	public void render(SchindlerMSeriesScreen2Even.BlockEntity blockEntity, float tickDelta, GraphicsHolder graphicsHolder1, int light, int overlay) {
	...
	private void renderLiftDisplay(StoredMatrixTransformations storedMatrixTransformations, World world , Lift lift ,float width,float width1,float height1,float height) {
		...
		if (!noFloorNumber || !noFloorDisplay) {
			...
			MainRenderer.scheduleRender(TextureList.instance.getSchindlerMSeriesScreen2EvenDisplay(text, 0xFF0000).identifier, false, QueuedRenderLayer.LIGHT_TRANSLUCENT, (graphicsHolder, offset) -> {
