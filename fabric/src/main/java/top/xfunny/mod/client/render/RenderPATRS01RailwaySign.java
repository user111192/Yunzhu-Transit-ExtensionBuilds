...
public class RenderPATRS01RailwaySign<T extends PATRS01RailwaySign.BlockEntity> extends BlockEntityRenderer<T> implements IBlock, IGui, IDrawing {
    ...
    public static float getMaxWidth(String[] signIds, int index, boolean right) {
        ...
        for (int i = index + (right ? 1 : -1); right ? i < signIds.length : i >= 0; i += (right ? 1 : -1)) {
            if (signIds[i] != null) {
                final SignResource sign = RenderPATRS01RailwaySign.getSign(signIds[i]);
